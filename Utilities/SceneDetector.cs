...
namespace UserContentIndexer.Utilities
{
    public class SceneDetector
    {
        ...
        public List<string> ProcessVideo(string videoPath)
        {
            try
            {
                ...
                while (capture.Read(currentFrame))
                {
                    ...
                    {
                        var (isSceneChange, metrics) = DetectSceneChange(previousFrame, currentFrame, DefaultThreshold, DefaultMinPixelDiff);
